...
export function SearchResults({ data }: { data: SearchResultsData }) {
  ...
  return <section className="os-search-results" aria-label="Library search results">
    ...
    {!data.query && <div className="os-search-empty"><strong>Start with a filename, folder, or file type</strong><span>Quick results will update progressively as you type.</span></div>}
    {data.query && !total && <div className="os-search-empty"><strong>No matches</strong><span>Try a shorter name or a different file type.</span></div>}
